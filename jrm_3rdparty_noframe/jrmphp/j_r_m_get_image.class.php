...
class JRMGetImage implements RocketSled\Runnable {
    ...
    public function get_image() {
        ...
        try {
            ...
            foreach ($sql as $s) {
                ...
                //$this->image_url($uid, $photocount, "LR", $id);
    ...
    public function prop_check($uid, $photocount, $resolution, $id) {
        ...
        for ($i = 1; $i <= $photocount; $i++) {
            ...
            $filepath = __DIR__ . "/../jrmimages/" . $filename;
            try {
                $chk_img = Plusql::from($this->profile)->img_res_prop->select("*")->where("image_path = '{$filename}'")->run()->img_res_prop;
            } catch (EmptySetException $e) {
                $this->img_ins($id, $filename);
            }
//            if (!file_exists($filepath)) 
//            {
//                $this->img_dwnld($filename, $filepath, $id);
//            }
        }
    }

//    public function max_img($uid, $photocount, $resolution, $id) {
//
//        for ($i = 1; $i <= $photocount; $i++) {
//            $count = 0;
//            $filename = $resolution . $uid . '-' . $i . '.jpg';
...
//                $chk_img = Plusql::from($this->profile)->img_res_prop->select("*")->where("image_path = '{$filename}'")->run()->img_res_prop;
//                if (!file_exists($filepath)) {
//                    $this->while_loop($uid, $photocount, $resolution, $id, $filename);
//                }
//            } catch (EmptySetException $e) {
//
//                $this->while_loop($uid, $photocount, $resolution, $id);
//            }
//        }
//    }
