<?php
include_once '../Config/central.class.php';
include_once '../Config/constants.class.php';
use Config\Central;
...
abstract class RSBase implements rocketsled\Runnable {
    ...
    public function update_single_slider($box_id, $col_name, $hmin_id, $hmax_id, $hcurr_id, $curr_id) { //$slider_id = min_bed,$col_name = _bedroom 
        try {
    ...
    public function update_double_slider($box_id, $col_name, $hmin_id, $hmax_id, $hcurr_id_min, $curr_id_min, $hcurr_id_max, $curr_id_max) { //$slider_id = min_bed,$col_name = _bedroom 
        try {
            $this->close_open_box($box_id);
            try {
                $min_bed = PluSQL::from($this->profile)->res_prop->select("res_prop_id,max(CAST({$col_name} as UNSIGNED)) as maxb, min(CAST({$col_name} as UNSIGNED)) as minb")->limit('0,1')->run()->res_prop;
    ...
    public function close_open_box($box_id) { //example $box_id = bedandbaths_val
        try {
            if (isset($_REQUEST[$box_id]) && $_REQUEST[$box_id] != "0") {
    ...
    public function getimage_values($sql_query, $moredetails, $repeat, $addr1, $addr2, $status, $margin, $listprice, $bed, $bath, $sqft, $imgsrc, $stop) {
        try {
            ...
            foreach ($sql as $s) {
                ...
                //$thumb_view->setValue($bed, number_format($s->_bedrooms) . ' Beds');
                //$thumb_view->setValue($bath, '|'.number_format($s->_full_baths) . ' Baths');
...
//                    }
//                    try {
//                        $primary_image = Plusql::from($this->profile)->img_res_prop->select("*")->where("res_prop_id= {$s->res_prop_id}  AND primary_pic = 1 AND image_path != 'default.jpg'")->orderby("img_res_prop_id")->limit('0,1')->run()->img_res_prop;
//                        $imagepath = PACKAGES_DIR . '/jrmimages/' . $primary_image->image_path;
//                        if (file_exists($imagepath)) {
//                            if (imagecreatefromjpeg($imagepath) !== FALSE) {
//                                $thumb_view->setValue($imgsrc . '@src', PACKAGES_DIR . '/jrmimages/' . $primary_image->image_path);
//                            } else {
//                                $thumb_view->setValue($imgsrc . '@src', PACKAGES_DIR . Constants::DEFAULT_IMAGE);
//                            }
//                        } elseif (!file_exists($imagepath)) {
//                            $this->img->img_dwnld($primary_image->image_path);
//                            $thumb_view->setValue($imgsrc . '@src', PACKAGES_DIR . '/jrmimages/' . $primary_image->image_path);
//                        }
//                    } catch (EmptySetException $e) {
//                        try {
...
//                            if (file_exists($imagepath)) {
//                                if (imagecreatefromjpeg($imagepath) !== FALSE) {
//                                    $thumb_view->setValue($imgsrc . '@src', PACKAGES_DIR . '/jrmimages/' . $sql_image->image_path);
//                                } else {
//                                    $thumb_view->setValue($imgsrc . '@src', PACKAGES_DIR . Constants::DEFAULT_IMAGE);
//                                }
//                            } elseif (!file_exists($imagepath)) {
//                                $this->img->img_dwnld($sql_image->image_path);
//                                $thumb_view->setValue($imgsrc . '@src', PACKAGES_DIR . '/jrmimages/' . $sql_image->image_path);
//                            }
//                        } catch (EmptySetException $e) {
//                            $thumb_view->setValue($imgsrc . '@src', PACKAGES_DIR . Constants::DEFAULT_IMAGE);
...
//                die($create_table);
//                if (substr($create_table, -10, -11) == "") {
//                    $create_table = substr($create_table, 0, -11) . "TEXT);";
//                }
    ...
    public function adv_img_dwnld($thumb_view, $sqlid, $photocount, $uid, $src_id) {
        try {
            $img_count = Plusql::from($this->profile)->res_prop->select("res_prop_id,count(*) as cnt")->where("res_prop_id= {$sqlid}")->limit('0,1')->run()->res_prop->cnt;
            if ($img_count != $photocount) {
                $this->img->prop_check($uid, $photocount, "HR", $sqlid);
            }
            try {
                $primary_image = Plusql::from($this->profile)->img_res_prop->select("*")->where("res_prop_id= {$sqlid}  AND primary_pic = 1")->orderby("img_res_prop_id")->limit('0,1')->run()->img_res_prop;
                $imagepath = PACKAGES_DIR . '/jrmimages/' . $primary_image->image_path;
                if (file_exists($imagepath)) {
                    if (imagecreatefromjpeg($imagepath) !== FALSE) {
                        $thumb_view->setValue('#' . $src_id . '@src', PACKAGES_DIR . '/jrmimages/' . $primary_image->image_path);
                    } else {
                        ...
                        if (file_exists($imagepath)) {
                            if (imagecreatefromjpeg($temp) === FALSE) {
                                throw new EmptySetException('CORRUPTED1: ' . $sqlid . ' ' . $uid . '<br/>');
                            } else {
                                $thumb_view->setValue('#' . $src_id . '@src', PACKAGES_DIR . '/jrmimages/' . $primary_image->image_path);
                            }
                        } else
                ...
                $sql_image = Plusql::from($this->profile)->img_res_prop->select("*")->where("res_prop_id= {$sqlid}")->orderby("img_res_prop_id")->limit('0,1')->run()->img_res_prop;
                $imagepath = PACKAGES_DIR . '/jrmimages/' . $sql_image->image_path;
                if (file_exists($imagepath)) {
                    if (imagecreatefromjpeg($imagepath) !== FALSE) {
                        $thumb_view->setValue('#' . $src_id . '@src', PACKAGES_DIR . '/jrmimages/' . $sql_image->image_path);
                    } else {
