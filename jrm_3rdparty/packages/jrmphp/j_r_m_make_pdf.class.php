...
class JRMMakePdf implements RocketSled\Runnable {
    ...
    public function make_pdf_files($s) {
        try {
            try {
                $sql4img = PluSQL::from($this->profile)->img_res_prop->select("*")->where("res_prop_id = {$s->res_prop_id} and primary_pic=1")->limit("0, 1")->run()->img_res_prop;
                $imagepath = "http://ntreispictures.marketlinx.com/MediaDisplay/".substr($s -> uid, -2)."/".$sql4img->image_path;
                $this->template->setvalue('#image@src', $imagepath);
            } catch (Exception $e) {
                try {
                    $sql4img = PluSQL::from($this->profile)->img_res_prop->select("*")->where("res_prop_id = {$s->res_prop_id} ")->limit("0, 1")->run()->img_res_prop;
                    $imagepath = "http://ntreispictures.marketlinx.com/MediaDisplay/".substr($s -> uid, -2)."/".$sql4img->image_path;
                    $this->template->setvalue('#image@src', $imagepath);
                } catch (Exception $e) {
...
//            $this->set_value('#FOUNDATION', 'FOUNDATION: ', strtoupper($s->foundation));
    ...
    public function pdf($sql4pdf, $ins = 1) {
        @mkdir(__DIR__ . "/../jrmpdf" );
        Central::delete_old_files(__DIR__ . "/../jrmpdf/" ); 
